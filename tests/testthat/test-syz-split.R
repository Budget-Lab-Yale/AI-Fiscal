...
#
# The fixture-level invariant tests elsewhere only check that YiL and YiK
# come out positive, so a sign error that swapped the labor and capital
# shares of pass-through profit (or routed wages into YiK) would pass
...
.syz_expected <- function(dt, pt, W_star) {
  ...
  YiL <- dt$wages + dt$sole_prop + dt$farm +
    (1 - sa_cs) * sa_net + (1 - psh) * sp_net +
    (1 - pa_cs) * pa_net + (1 - psh) * pp_net
  YiK <- dt$txbl_int + dt$exempt_int + dt$div_ord + dt$div_pref +
...
test_that("YiL / YiK match an independent SYZ recomputation (sign-locked)", {
  dt  <- .syz_fixture()
  out <- apply_passthrough_split(data.table::copy(dt), .syz_params)
  exp <- .syz_expected(dt, .syz_params$raw$passthrough, W_star = 100)
  expect_equal(out$YiL, exp$YiL, tolerance = 1e-9)
  expect_equal(out$YiK, exp$YiK, tolerance = 1e-9)
})

test_that("wages flow only to YiL; pure capital flows only to YiK", {
  dt  <- .syz_fixture()
  out <- apply_passthrough_split(data.table::copy(dt), .syz_params)
  # Unit A: wages = 100, nothing else.
  expect_equal(out$YiL[1], 100, tolerance = 1e-9)
  expect_equal(out$YiK[1], 0,   tolerance = 1e-9)
  # Unit B: txbl_int 200 + div_ord 10 + div_pref 20 + kg_lt 30 = 260.
  expect_equal(out$YiL[2], 0,   tolerance = 1e-9)
  expect_equal(out$YiK[2], 260, tolerance = 1e-9)
})
...
test_that("below-W* active profit splits 25% capital / 75% labor (not flipped)", {
  ...
  # is active_below_capital_share = 0.25, so YiK gets 20 and YiL gets 60.
  # A labor<->capital flip would give YiK = 60, YiL = 20.
  expect_equal(out$YiK[3], 0.25 * 80, tolerance = 1e-9)
  expect_equal(out$YiL[3], 0.75 * 80, tolerance = 1e-9)
})

test_that("the split conserves mass: YiL + YiK == sum of source components", {
  dt  <- .syz_fixture()
  out <- apply_passthrough_split(data.table::copy(dt), .syz_params)
  components <- with(dt,
  ...
  expect_equal(out$YiL + out$YiK, components, tolerance = 1e-9)
})
