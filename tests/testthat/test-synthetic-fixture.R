...
# apply_passthrough_split() produces a plausible YiL / YiK split, so
...
test_that("synthetic fixture loads and matches PUF schema", {
  ...
  # SYZ split runs and produces non-trivial YiL + YiK
  dt2 <- apply_passthrough_split(dt, params)
  expect_true(all(c("YiL", "YiK") %in% names(dt2)))
  expect_gt(sum(dt2$weight * dt2$YiL), 0)
  expect_gt(sum(dt2$weight * dt2$YiK), 0)
})
