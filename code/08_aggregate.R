...
.build_scenario_guide <- function(publishable = FALSE) {
  dt <- data.table(
    ...
    mechanism = c(
      ...
      "5-yr cumulative shock target",
      "5-yr cumulative shock target",
      "Karger labor-share decline: theta1_k from yaml",
      "Hold labor-capital share at baseline: theta1_k := theta0_k",
      "Log-affine, lambda_S2 = 1 - k_inequality * g_y",
      "Uniform scaling",
      "Log-affine, lambda_S3 = 1 + k_inequality * g_y",
      "All gross gains realized in-year"
    ),
    formula = c(
      "g_k = (theta1_k*(1+g_y) - theta0_k)/theta0_k; g_l = (theta1_l*(1+g_y) - theta0_l)/theta0_l",
      "(same as S)",
      "(same as S)",
      "theta1_k from yaml shock.variants.<variant>.theta1_k; g_k and g_l derived as above.",
      "theta1_k = theta0_k  =>  g_k = g_l = g_y; labor and capital both grow at g_y.",
      "ln(y_l1) = mu1 + lambda_S2 * (ln(y_l) - mu0); lambda_S2 = 1 - k_inequality * g_y; mu1 via uniroot to hit Y1^L$",
      "y_l1 = rho * y_l, rho = Y1^L$ / Y0^L$",
      "ln(y_l1) = mu1 + lambda_S3 * (ln(y_l) - mu0); lambda_S3 = 1 + k_inequality * g_y; mu1 via uniroot to hit Y1^L$",
      "X_ltcg_V1 = X_ltcg_gross"
    ),
    defaults_implied = c(
      "theta1_k=0.450, g_y=0.0060 -> g_k ≈ +1.7%,  g_l ≈ -0.3%  (Slow)",
      "theta1_k=0.462, g_y=0.0358 -> g_k ≈ +7.5%,  g_l ≈ +0.4%  (Moderate)",
      "theta1_k=0.487, g_y=0.0716 -> g_k ≈ +17.3%, g_l ≈ -0.9% (Rapid)",
      "Canonical AI-growth-plus-reallocation shock.",
      "Paired twin of R: same g_y, labor and capital both scale by (1+g_y).",
      "k_inequality = 1 by default (config: labor_inequality.k). See labor_inequality sheet for per-variant lambda_S2.",
      "—",
      "k_inequality = 1 by default (config: labor_inequality.k). See labor_inequality sheet for per-variant lambda_S3.",
      "—"
    ),
    notes = c(
...
.build_labor_inequality <- function(publishable = FALSE) {
  dt <- data.table(
    ...
    symbol = c(
      "k_inequality",
      "lambda_S2 = 1 - k_inequality * g_y",
      "lambda_S3 = 1 + k_inequality * g_y",
      "mu1 via uniroot s.t. sum(weight * exp(mu1 + lambda * (ln(y_l) - mu0))) = Y1^L$"
    ),
    default_value = c(
      "1.0",
    ...
    meaning = if (publishable) c(
      ...
      "Under the compressive labor scenario (S2), top earners are pulled toward the mean. This is the slope of the log-affine map applied to baseline log labor income; values below 1 compress the distribution. Calculated as 1 - k_inequality * (AI growth bump g_y).",
      "Under the expansive labor scenario (S3), top earners pull further away from the mean. This is the slope of the log-affine map applied to baseline log labor income; values above 1 stretch the distribution. Calculated as 1 + k_inequality * (AI growth bump g_y).",
      ...
      "Multiplier on the AI growth bump g_y that sets the size of the dispersion shift in the log-affine map. Same value across all variants in a run; per-variant lambda values differ because g_y differs.",
      "Compressive map: top of the ln(y_l) distribution is pulled toward the mean by (1 - lambda_S2). Rationale: AI commoditizes high-skill premia. Larger g_y => more compression.",
      "Expansive map: top of the ln(y_l) distribution is stretched away from the mean by (lambda_S3 - 1). Rationale: AI super-charges top earners (winner-take-most). Larger g_y => more expansion.",
      "Intercept mu1 is solved to preserve the aggregate labor income target Y1^L$ from the macro step. mu0 is the weighted mean of ln(y_l) on positive-labor units in the baseline."
    ),
    source = c(
      "scenario_params.yaml: labor_inequality.k (default 1).",
      "Computed in 03_shock_labor.R; g_y from the macro step (per-variant). See parameters sheet.",
      "Computed in 03_shock_labor.R; g_y from the macro step (per-variant). See parameters sheet.",
...
.build_corporate_tax <- function(publishable = FALSE) {
  dt <- data.table(
    ...
    symbol = c(
      ...
      "eta = tau_stat * (Y0^K$ * kappa_corp) / CBO_CIT$",
      "X = Y0^K$ * g_k",
      "X_to_units = X",
      "delta_R_CIT = tau_stat * kappa_corp * X / eta ≡ X * CBO_CIT$ / Y0^K$",
    ...
    default_value = c(
      ...
      "Calibrated at runtime from Y0^K$",
    ...
    meaning = if (publishable) c(
      ...
      "Single calibrated scale factor that absorbs both the gap between household-realized capital income (Y0^K$) and the pre-realization corporate tax base, and the gap between the statutory rate and the effective rate (avoidance, NOLs, credits, profit shifting). One value per (variant, share_mode) cell.",
...
.friendly_parameters_sheet <- function(dt) {
  rename_map <- c(
    ...
    theta1_k                   = "Post-shock capital share (theta1_k)",
    g_y                        = "AI growth bump over CBO baseline (g_y)",
    g_k                        = "Capital income growth rate (g_k)",
    g_l                        = "Labor income growth rate (g_l)",
    k_inequality               = "Inequality multiplier (k_inequality)",
    lambda_S2                  = "Compressive slope (lambda_S2)",
    lambda_S3                  = "Expansive slope (lambda_S3)",
    y0_l_dollar                = "Baseline labor income, $ (Y0^L)",
    y0_k_dollar                = "Baseline capital income, $ (Y0^K)",
...
.build_variable_list_microsim <- function(include_decomp = FALSE) {
  base <- data.table(
    ...
    column = c("scenario_id", "instrument", "baseline", "counterfactual", "delta",
               ...
               "variant", "share_mode",
               "theta1_k", "g_y", "g_k", "g_l",
               "k_inequality", "lambda_S2", "lambda_S3",
               "y0_l_dollar", "y0_k_dollar", "y0_dollar", "X", "X_to_units",
    ...
    description = c(
      ...
      "Factor-share mode. R = reallocate (Karger theta1_k from yaml). F = fixed (theta1_k := theta0_k so the labor-capital split is preserved). Same g_y under both modes; F has g_k = g_l = g_y by construction.",
      "Post-shock capital share target = 1 - post-shock labor share.",
      ...
      "Cumulative capital-income growth rate over the horizon; derived in 02_params.R from (theta1_k, g_y, theta0_l).",
      "Cumulative labor-income growth rate over the horizon. Labor grows at g_l (equals g_k under fixed share; below g_k under reallocate).",
      ...
      "S2 (compressive) log-affine slope = 1 - k_inequality * g_y. Per-variant since g_y varies.",
      "S3 (expansive) log-affine slope = 1 + k_inequality * g_y. Per-variant since g_y varies.",
      "Microsim baseline aggregate labor income, sum(weight*y_l) in $.",
      "Microsim baseline aggregate capital income, sum(weight*y_k) in $.",
      "y0_l_dollar + y0_k_dollar.",
      "Implied capital flow = y0_k_dollar * g_k ($).",
      ...
      "Calibrated corporate-base scale factor (eta_corp). cit_statutory * (y0_k_dollar * kappa_corp) / cbo_cit_baseline_dollar. Absorbs both household-realized-vs-pre-realization wedge and the statutory-vs-effective gap.",
      "Off-microsim CIT revenue change ($). cit_statutory * kappa_corp * X / eta_corp; algebraically equals X * cbo_cit_baseline_dollar / y0_k_dollar. Add to microsim total in 09_tables_figures.R.",
